...
Block ColumnFileSetWithVectorIndexInputStream::readImpl(FilterPtr & res_filter)
{
    ...
    while (cur_column_file_reader != reader.column_file_readers.end())
    {
        ...
        if (tiny_readers[current_file_index] != nullptr)
        {
            ...
            auto selected_row_begin = std::find_if(selected_rows.cbegin(), selected_rows.cend(), [&](const auto & row) {
                return row.key >= read_rows;
            });
            auto selected_row_end = std::find_if(selected_row_begin, selected_rows.cend(), [&](const auto & row) {
                return row.key >= read_rows + file_rows;
            });
