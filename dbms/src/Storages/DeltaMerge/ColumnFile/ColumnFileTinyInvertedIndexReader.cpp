...
InvertedIndexViewerPtr ColumnFileTinyInvertedIndexReader::loadInvertedIndex(const ColumnValueSetPtr & set)
{
    ...
    const auto info_iter = std::find_if(index_infos->cbegin(), index_infos->cend(), [index_id](const auto & info) {
        return std::visit([&](const auto & index) -> bool { return index.index_id() == index_id; }, info.index_pros);
    });
    if (info_iter == index_infos->cend()
        || !std::holds_alternative<dtpb::InvertedIndexFileProps>(info_iter->index_pros))
        return nullptr;
    const auto & inverted_index = std::get<dtpb::InvertedIndexFileProps>(info_iter->index_pros);
    ...
    auto load_from_page_storage = [&]() {
        ...
        return InvertedIndexViewer::view(type_id, compressed, inverted_index.index_bytes());
    };
...
ColumnFileTinyInvertedIndexReader::~ColumnFileTinyInvertedIndexReader()
{
    LOG_DEBUG(
        log,
        "Finish vector search over column tiny_{}(rows={}){} cached, returned {} rows, cost_search={:.3f}s",
