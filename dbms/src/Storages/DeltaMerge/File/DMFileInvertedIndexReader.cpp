...
{
    return fmt::format(
        "index_size={} load={:.2f}s{}{}, search={:.2f}s, selected_nodes={}",
        index_size,
        duration_load_index,
        has_s3_download ? " (S3)" : "",
...
InvertedIndexViewerPtr DMFileInvertedIndexReader::loadInvertedIndex(const ColumnValueSetPtr & set)
{
    ...
    const auto index_file_path = dmfile->invertedIndexPath(index_id);
    ...
    if (auto s3_file_name = S3::S3FilenameView::fromKeyWithPrefix(index_file_path); s3_file_name.isValid())
    {
        ...
        for (auto i = 3; i > 0; --i)
        {
            try
            {
                if (auto file_guard = file_cache->downloadFileForLocalRead(s3_file_name, index_props.index_bytes());
                    file_guard)
                {
