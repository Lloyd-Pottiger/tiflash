...
try
{
    ...
    // Prepare DMFile
    {
        ...
        }
        {
            ...
            // delete the 7th row, but DMFileInvertedIndexReader should not care about it
    ...
    // test col > 1
    {
        ...
        auto column_value_set = rs_operator->buildSets(snapshot);

        DMFileInvertedIndexReader reader(column_value_set, dm_file, nullptr);
...
try
{
    ...
    // Prepare DMFile
    {
        ...
        {
            Block block = DMTestEnv::prepareSimpleWriteBlockWithNullable(0, 3);
            block.insert(createColumn<UInt64>({1, 2, 3}, cols->at(0).name, integer_cd_1.id));
            block.insert(createColumn<Int64>({1, 2, 3}, cols->at(1).name, integer_cd_2.id));
            stream->write(block, DMFileBlockOutputStream::BlockProperty{0, 0, 0, 0});
        }
        {
            Block block = DMTestEnv::prepareSimpleWriteBlockWithNullable(3, 6);
            block.insert(createColumn<UInt64>({4, 5, 6}, cols->at(0).name, integer_cd_1.id));
            block.insert(createColumn<Int64>({4, 5, 6}, cols->at(1).name, integer_cd_2.id));
            stream->write(block, DMFileBlockOutputStream::BlockProperty{0, 0, 0, 0});
        }
        {
            Block block = DMTestEnv::prepareSimpleWriteBlockWithNullable(6, 9);
            block.insert(createColumn<UInt64>({7, 8, 9}, cols->at(0).name, integer_cd_1.id));
            block.insert(createColumn<Int64>({7, 8, 9}, cols->at(1).name, integer_cd_2.id));
            // delete the 7th row, but DMFileInvertedIndexReader should not care about it
            stream->write(block, DMFileBlockOutputStream::BlockProperty{0, 1, 0, 0});
        }
        stream->writeSuffix();
